...
MeshData OBJLoader::Load(const char* filename, ID3D11Device* _pd3dDevice, bool invertTexCoords)
{
	...
	if (!binaryInFile.good())
	{
		...
		while (!inFile.eof()) //While we have yet to reach the end of the file...
		{
			...
			else if (input.compare("f") == 0) //Face
			{
				...
				for (int i = 0; i < 3; ++i)
				{
					...
					//Minus 1 from each as these as OBJ indexes start from 1 whereas C++ arrays start from 0
					textureIndices.push_back(tInd[i] - 1);
					//which is really annoying. Apart from Lua and SQL, there's not much else that has indexing
					normalIndices.push_back(nInd[i] - 1);
					//starting at 1. So many more languages index from 0, the .OBJ people screwed up there.
				}
			}
		}
		...
		auto finalVerts = new SimpleVertex[meshVertices.size()];
		...
		auto indicesArray = new unsigned short[meshIndices.size()];
	...
	auto finalVerts = new SimpleVertex[numVertices];
	auto indices = new unsigned short[numIndices];
